...
@app.command
def add(title: str,description:str = typer.Arguement(""),due_date:Optional[str]= None):
...
def update_status(id:int,status:str):
    ...
    valid_status = [s for s in TaskStatus]

    if status not in valid_status:
        typer.echo(f"Invalid status. Choose either of these {valid_status}")
        raise typer.Exit()
    
    
    db.update_status_task(title,status)
...
def list_task(status: Optional[str]= None):
    if status:
        ...
        if status not in valid_status:
            typer.echo(f"Invalid status command. Try with these {status}")
            raise typer.Exit()
